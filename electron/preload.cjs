...
const legacyRecord = (record) => ({
  ...record,
  query: record.sources?.find((source) => source.query)?.query || record.title,
  path: true,
});
async function importDropped(files) {
  ...
  const session = await ipcRenderer.invoke(
    ...
    prepared.map((file) => ({
      name: file.name,
      size: file.bytes instanceof Uint8Array ? file.bytes.byteLength : -1,
    })),
  );
  const records = [],
...
contextBridge.exposeInMainWorld('scout', {
  ...
  configure: call('settings:set'),
  search: call('images:search'),
  save: call('images:save'),
  imageBytes: call('images:bytes'),
  library: () =>
    ipcRenderer
      .invoke('vault:list')
      .then((records) => records.map(legacyRecord)),
  reveal: call('vault:reveal'),
  hide: call('window:hide'),
  quit: call('window:quit'),
  vault: {
    info: call('vault:info'),
    import: call('vault:import'),
    importDropped,
    beginDrop: call('vault:drop-begin'),
    importDropFile: call('vault:drop-file'),
    list: call('vault:list'),
    copy: call('vault:copy'),
    export: call('vault:export'),
    reveal: call('vault:reveal'),
    openFolder: call('vault:open-folder'),
    delete: call('vault:delete'),
    bytes: call('vault:bytes'),
  },
  onFocus: (callback) => {
